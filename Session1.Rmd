...
Suppose we wanted to know whether there are more fatalities in countries with a higher overall number of conflicts. Let's first look at the distribution of number of conflicts.
```{r}
summary(mydata$nconflict)
```
...
We can display two indicators in the same boxplot. We can use this feature to answer the question whether states with more conflict events also see more overall fatalities (which is, as expected, the case).
```{r, fig.width = 3, fig.height= 3}
boxplot(mydata$fatalities ~ mydata$nconflict_high)
```
...
#setwd("/Users/thereseanders/Projects")
```
